...
if [ "${SSL_ENABLED}" = "1" ]; then
    ...
    if [ ! -f "$CA_DIR/ca.crt" ] || [ ! -f "$CA_DIR/ca.key" ]; then
        ...
        if ! openssl req -new -newkey rsa:4096 -days 3650 -nodes -x509 \
            ...
            echo "[lancache] ERROR: CA certificate generation failed" >&2
            exit 1
        fi
        echo ""
        echo "╔══════════════════════════════════════════════════════════════════╗"
    ...
    _sign_cert() {
        ...
        if ! openssl req -new -newkey rsa:2048 -nodes -subj "/CN=${cn}" \
            ...
            echo "[lancache] ERROR: CSR/key generation failed for ${cn}" >&2
            exit 1
        fi
        ...
        if [ -n "$ext" ]; then
            if ! openssl x509 -req -days 3650 \
                ...
                -out "$crt"; then
                echo "[lancache] ERROR: Certificate signing failed for ${cn}" >&2
                exit 1
            fi
        else
            if ! openssl x509 -req -days 3650 \
