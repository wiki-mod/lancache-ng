...
if [ "${SSL_ENABLED}" = "1" ]; then
    ...
    # call, without needing to run the rest of this entrypoint (bug-hunt
    # #849, finding #9's second sub-part: no test previously asserted
    # ca.key's/CERT_DIR's file modes at all, which had already become a live
    # regression risk once #1031's chmod fix landed with zero coverage
    # guarding it against a future accidental deletion).
    _ensure_ca_cert() {
...
# (PROXY_ALLOWED_CLIENT_CIDRS) map generation directly. Neither had any test
# coverage before this file -- both the strict-mode 403 code path and the
# CIDR-allowlist 403 code path had zero automated coverage anywhere in the
# suite (bug-hunt #849, finding #9's fourth sub-part).
_render_ssl_map() {
