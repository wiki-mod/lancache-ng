...
# target ":443" for an empty SNI, silently reintroducing #88 for the
# project's own shipped default mode. Emitting
# this as an explicit "" key once, shared by both branches below, means a
...
# so tests/bats/proxy_stream_backend_map.bats can drive the real N4 fix
# directly, for both PROXY_SECURITY_MODE branches.
...
# anywhere. Fixed with ngx_stream_access_module's plain allow/deny
...
# Factored into its own function (rather than inline top-level script code)
# so tests/bats/proxy_stream_client_acl.bats can drive the real N1 fix
# directly.
...
# for the very first bad boot after such an upgrade -- exactly the boot this
# rollback protection exists for. Fixed with a one-time backfill, scoped to
...
# 00-stream-client-acl.conf gets that one file copied in from what this boot
# just generated. Safe specifically because that file's content is a pure,
# deterministic function of PROXY_ALLOWED_CLIENT_CIDRS alone (see "2b."
# above) -- unlike the other candidate files, it carries no information
# about whether this boot's cdn-domains.txt or templates are valid, so
# backfilling it does not change what the snapshot's original files actually
# validated together at the time they were taken. Idempotent (plain
# overwrite) and a no-op for any snapshot that already has the file
# (already-migrated, or created new under the current candidate list).
_migrate_legacy_proxy_snapshots_for_stream_acl() {
    local snapshot_root="$1" acl_file="$2"
    local id snap_dir
    while IFS= read -r id; do
        [ -n "$id" ] || continue
        snap_dir="${snapshot_root}/${id}"
        ...
        if cp -p "$acl_file" "${snap_dir}/00-stream-client-acl.conf" 2>/dev/null; then
            kgs_log MIGRATE "proxy" "backfilled 00-stream-client-acl.conf into pre-existing known-good snapshot $id"
        else
...
_migrate_legacy_proxy_snapshots_for_stream_acl "$PROXY_CONFIG_SNAPSHOT_DIR" "$STREAM_CLIENT_ACL_FILE"
