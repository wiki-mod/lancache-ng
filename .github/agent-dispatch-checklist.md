...
20. **New file = formal DISACK by default.** Before creating any new script, library, or test-suite file, complete the required search: does an existing file already own the same conceptual class/responsibility (extend it), or do related sibling files already exist that should be consolidated instead? State the result of this check in the PR. The DISACK lifts only by extending an existing file, or by a documented check plus the maintainer's explicit ACK obtained *before* the file is created — a coordinating agent cannot grant this on the maintainer's behalf regardless of circumstance, and must relay the request upward instead. (AG-CODE-013)
...
11. **Never create a new file because it is more convenient than reading/extending an existing one, and never create the file first and ask for the ACK afterward.** A plausible-sounding reason for a new file is not itself an ACK, and neither is a coordinating agent's own sign-off, regardless of circumstance — only the maintainer can lift this DISACK. If genuinely uncertain whether an existing file should be extended instead, stop and relay the question up to the maintainer — do not create the file while that is unresolved. (AG-CODE-013)
