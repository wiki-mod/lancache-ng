...
- Added an Admin UI reader for the central syslog-ng log store (#633,
  ...
  way.
- Added active NATS connection termination for a removed/rotated secondary
  (#681, hardening follow-up to #621/#583's own documented "next reconnect
  only" gap): `services/ui/src/nats_kick.rs` looks up (CONNZ) and
  force-disconnects (`$SYS.REQ.SERVER.*.KICK`) a secondary's live NATS
  connection immediately after `remove_secondary`/`rotate_token` commit their
  DB write, instead of relying solely on the secondary's own next reconnect
  attempt or its issued user JWT's 90-day expiry. Requires a new NATS system
  account (`services/nats/nats.conf`'s first-ever `accounts {}` block; new
  `NATS_SYS_USER`/`NATS_SYS_PASSWORD` static role, generated by `setup.sh`
  the same way as the other static NATS roles) -- confirmed additive: every
  existing role and every callout-authenticated secondary still lives in the
  implicit default account (`$G`), unaffected. The kick runs as a background,
  best-effort task so a slow/unreachable NATS system account never adds
  latency to the Admin UI's HTTP response; the no-live-connection case (the
  common one) is a graceful no-op, not an error. Validated end-to-end against
  a real `nats-server` 2.14.3 (the exact pinned version) and a real,
  held-open `nats-subscriber` connection in
  `scripts/nats-secondary-auth-callout-simulation.sh`, polling nats-server's
  own `connz` monitor endpoint to prove the disconnect actually happened
