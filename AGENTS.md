...
- **Scope**: CodeQL Rust extractor "macro expansion failed" warnings for ordinary macro invocations (`format!`, `assert_eq!`, `vec!`, `json!`, `tracing::*`, etc.) sitting in normal, human-authored source — the everyday case this repository actually hits (e.g. a `format!()` call in a regular function).
- **Reason**: CodeQL's Rust extractor (for which `build-mode: none` is the only supported Rust mode) emits these `macro expansion failed` warnings as a known limitation of its upstream `rust-analyzer`-based extraction, not because of any defect in this repository's code. Because the warnings fire on ordinary macro invocations in hand-written source, a strict, unscoped reading of the warnings-as-errors rule would block CodeQL on essentially every Rust PR and delay legitimate security fixes, even though the compiled binary and test behavior are correct. `.github/workflows/codeql.yml`'s extraction-quality-signal step documents this same limitation and prints a standing caveat on every run.
- **Tracking**: issue #394. This exception was approved as part of the v0.2.0 security hardening pass.
- **Validation**: CodeQL analysis must still run, and every finding that is not one of these carved-out extraction warnings is still treated as blocking. The exception only narrows which findings are non-blocking. A CodeQL exception pull request must demonstrate that the finding is one of these named extraction warnings — a `macro expansion failed` warning on an ordinary macro invocation in human-authored source — that it is a known upstream false positive rather than a real defect, and that the instance stays referenced in #394.
- **Non-Expansion**: This exception applies only to these named CodeQL Rust macro-expansion extraction warnings. It does not cover other languages, other security checks (e.g., cargo-audit, SAST linters), `cargo check`/`cargo clippy` warnings (which remain hard failures), or any genuine CodeQL finding that is not one of these upstream macro-expansion extraction warnings.
