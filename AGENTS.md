...
2. Explicitly report back—by name—that it has read all three files: state "I have read AGENTS.md, CLAUDE.md, and .github/AGENTS.md" verbatim or equivalent, not merely "understood" or "acknowledged."
3. The parent/main thread that spawned it must confirm this acceptance is genuine, not rote, by asking the sub-agent to cite or quote at least one specific rule (by its `[AG-...]` ID) from the files it claims to have read, before trusting any of its subsequent work.
...
| AG-GH-013 | Actively maintain issues/PRs with comments as work happens, not only a one-time body | Manual review |
...
| AG-WF-024 | Sub-agents need their own verified-clean working directory; never trust inherited/recycled worktree state; force-push forbidden; sub-agents must read+accept governance and report back by name; check back before destructive actions | Manual review |
