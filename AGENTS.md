...
- Treat warnings as errors for repository work. Do not list a check as successful when it emitted warnings, failed setup, or used a broken fallback. Exception: known, upstream-tracked CodeQL Rust macro-expansion extraction warnings (tracked in issue #394) do not by themselves block a PR, since they are a documented CodeQL/rust-analyzer limitation rather than a defect in this repository's code — but any such warning must remain referenced in #394 and periodically reevaluated, not silently normalized as acceptable everywhere.
