...
- **distcc/pump**: Rust service builders that install `distcc` must receive host lists through BuildKit secrets or trusted CI variables, never hardcoded Dockerfile values. When enabled, they must set `CC=distcc`, `GCC=distcc`, `CXX=distcc`, and put `/usr/lib/distcc` at the front of `PATH` so direct `cc`, `gcc`, `c++`, and `g++` calls are intercepted. `distcc-pump` host lists must include at least one `,cpp` host entry and must fail closed if pump startup fails. Any image that installs Debian `distcc-pump` must patch the known invalid Python regex escapes before package configuration and verify the result with `python3 -Werror::SyntaxWarning`.
- **Build parallelism**: Cargo and Docker Rust builds must use one project-wide job rule unless a PR justifies an override: detected CPU cores minus two, with a minimum of two jobs. Do not hardcode service-local values such as `CARGO_BUILD_JOBS=6`.
