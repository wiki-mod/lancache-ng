...
- **Docker builds**: production/service Dockerfiles use multi-stage builds with pinned `rust:slim` builder images. Do not use `rust:latest` or Debian-based builder images for production/service Dockerfiles. Local developer helper scripts and the future build-tools image intentionally use `rust:latest` by default when the image is explicitly overrideable; this keeps developer and CI validation tooling current while remaining separate from production service image pinning.
- **Build-tools image**: the planned build-tools image should use `rust:latest` intentionally, then install and smoke-test required tools such as `rustfmt`, `clippy`, and `sccache`. It must explicitly set and verify `PATH`, especially `/usr/local/cargo/bin`, to avoid false `command not found` failures.
