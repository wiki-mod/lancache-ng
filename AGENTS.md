...
- Excluded: `.md` files, `.env`/`.env.example` files, lockfiles (`Cargo.lock`), `.gitkeep`, the root `VERSION` file, vendored/generated build artifacts, and any file whose content is JSON or another comment-free structured format regardless of its extension (see above for concrete examples of each).
