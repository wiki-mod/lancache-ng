...
#
# Coverage for scripts/tracked/check-pr-diff-file-headers.sh: exercises the real
...
setup() {
    script="$BATS_TEST_DIRNAME/../../scripts/tracked/check-pr-diff-file-headers.sh"
    check_file_headers_script="$BATS_TEST_DIRNAME/../../scripts/tracked/check-file-headers.sh"
    ...
    # A stand-in for the real script this repo's own scripts/tracked/
    ...
    cp "$check_file_headers_script" "$work_dir/scripts/tracked/check-file-headers.sh"
    cp "$script" "$work_dir/scripts/tracked/check-pr-diff-file-headers.sh"
...
@test "passes silently when the diff is empty (base and head are the same commit)" {
    head_sha="$base_sha"
    run bash -c "cd '$work_dir' && SPDX_BASE_SHA='$base_sha' SPDX_BASE_REF=main GITHUB_SHA='$head_sha' bash scripts/tracked/check-pr-diff-file-headers.sh"
    [ "$status" -eq 0 ]
...
EOF
    ...
    run bash -c "cd '$work_dir' && PATH='$fixture_root/bin:$PATH' SPDX_BASE_SHA='$base_sha' SPDX_BASE_REF=main GITHUB_SHA='$base_sha' bash scripts/tracked/check-pr-diff-file-headers.sh"
    [ "$status" -eq 1 ]
    [[ "$output" == *"git diff\` itself failed"* ]]
}

@test "fails closed with a clear diagnostic when a required environment variable is missing" {
    run bash -c "cd '$work_dir' && SPDX_BASE_REF=main GITHUB_SHA='$base_sha' bash scripts/tracked/check-pr-diff-file-headers.sh"
    [ "$status" -ne 0 ]
