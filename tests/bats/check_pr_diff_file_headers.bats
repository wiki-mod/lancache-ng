...
#
# Coverage for scripts/check-pr-diff-file-headers.sh: exercises the real
...
setup() {
    script="$BATS_TEST_DIRNAME/../../scripts/check-pr-diff-file-headers.sh"
    check_file_headers_script="$BATS_TEST_DIRNAME/../../scripts/check-file-headers.sh"
    ...
    # A stand-in for the real script this repo's own scripts/ directory
    # points at, so check-pr-diff-file-headers.sh's own
    ...
    cp "$check_file_headers_script" "$work_dir/scripts/check-file-headers.sh"
    cp "$script" "$work_dir/scripts/check-pr-diff-file-headers.sh"
...
@test "fails closed with a clear diagnostic when a required environment variable is missing" {
    run bash -c "cd '$work_dir' && SPDX_BASE_REF=main GITHUB_SHA='$base_sha' bash scripts/check-pr-diff-file-headers.sh"
    [ "$status" -ne 0 ]
