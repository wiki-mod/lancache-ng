...
# `_dns_auth_validate_snapshot_or_rollback` adapter functions without
# executing the full entrypoint (PDNS_API_KEY placeholder checks, zone
# creation, RPZ generation, etc.).
...
load_dns_known_good_snapshot_helpers() {
...

    {
        ...
        # real entrypoint does), and the two DNS-specific adapter functions
        # further down. Mirrors the technique in
