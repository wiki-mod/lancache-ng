...
# `_fix_chrony_dir_ownership_core`, `fix_chrony_dir_ownership`,
# `clock_control_available`) directly, without executing the rest of the
# entrypoint (the fail-closed empty-upstream-list check that would `exit 1`
# under bats, and the final `exec chronyd`, which does not exist in the
# test environment).
...
load_ntp_entrypoint_helpers() {
    ...
    awk '
        ...
        in_fn7 { print }
        in_fn7 && /^\}$/ { in_fn7 = 0 }
        /^clock_control_available\(\) \{/ { in_fn8 = 1 }
        in_fn8 { print }
        in_fn8 && /^\}$/ { in_fn8 = 0 }
