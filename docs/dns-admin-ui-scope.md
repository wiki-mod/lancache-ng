...
| Recursor cache flush for a specific name | Internal helper (`flush_recursor_cache`, called by the add/remove routes above) | PowerDNS Recursor `cache/flush?domain=` API call, plus a NATS `lancache.dns.flush` broadcast so every recursor instance (not just the one this UI process talks to) drops its cached answer for that exact name. The broadcast now also carries the changed record's expected state (`zone`/`record_type`/`expected_content`); each subscriber confirms its local PowerDNS has converged to that state (AXFR landed) before flushing, deferring via NATS redelivery (up to a delivery ceiling) if not yet converged, so a flush never re-caches a stale answer ahead of propagation (issue #1095) |
