...
| Global `dnsupdate-require-tsig` toggle (issue #815 follow-up, default off) | `POST /domains/dnsupdate-require-tsig` (`toggle_dnsupdate_require_tsig`) | Writes/removes a marker file on the shared `powerdns-state` volume (same volume as the AAAA filter above, different file), but unlike that filter this is a static `pdns.conf` startup directive PowerDNS only reads once — the route therefore also restarts both `dns-standard` and `dns-ssl` (`docker_client::restart_service`) so the change actually takes effect, and entrypoint.sh's `render_template_atomic` extra-sed mechanism flips `dnsupdate-require-tsig=no` to `=yes` in the rendered `pdns.conf` on that restart. The route refuses to enable it (redirects with an error instead) unless a real `DDNS_TSIG_KEY` is already present on the shared-secrets volume — enabling it with no real key would reject every DNS UPDATE, signed or not. **Real-tested scope note**: for this project's actual zones, this global setting is defense-in-depth on top of protection PowerDNS's own per-zone `TSIG-ALLOW-DNSUPDATE` metadata already provides whenever a real TSIG key exists (`configure_ddns_tsig()` sets that metadata unconditionally, and a real key exists essentially always thanks to `resolve_shared_secret`'s auto-generation) — toggling this setting produces no observable behavior difference against `lan.`/the reverse zones in the common case, confirmed live with `nsupdate` against both signed and unsigned packets in both toggle states. See `services/dns/pdns.conf.template`'s own header comment on this line for the full writeup |
...
- TSIG key/metadata management via the UI. This is config-file-only by design
  ...
  **Narrower distinction, added when `dnsupdate-require-tsig` moved to 1b
  (issue #815 follow-up, 2026-08-06):** this exclusion is about key
  *material* and per-key metadata specifically — `dnsupdate-require-tsig`
  itself is a plain boolean (require TSIG globally, yes/no), carries no
  secret, and does not touch which keys exist or which zones they're
  attached to, so it does not fall under this exclusion the way rotating
  `DDNS_TSIG_KEY` would.
