...
Since this document was written, a small piece of that idea landed for individual services: see [known-good-config-snapshots.md](known-good-config-snapshots.md) for the automatic, per-service known-good configuration snapshot mechanism (nginx, dnsmasq, and PowerDNS's static `pdns.conf`/`recursor.conf` today). It is deliberately separate from this backup/restore contract — it is not a replacement, does not cover the whole install, and its snapshot volumes are not part of the automated backup manifest above. PowerDNS's zone/record database (`pdns.sqlite3`) is not covered by either mechanism yet; see known-good-config-snapshots.md's "Zones, records, and TSIG/DDNS metadata (deferred)" section.
