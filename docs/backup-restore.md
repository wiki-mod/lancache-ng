...
Since this document was written, a small piece of that idea landed for individual services: see [known-good-config-snapshots.md](known-good-config-snapshots.md) for the automatic, per-service known-good configuration snapshot mechanism (nginx, dnsmasq, and Kea DHCP today; Kea's is operator-selected from the Admin UI rather than automatic, since it has no container-startup moment to trigger from). It is deliberately separate from this backup/restore contract — it is not a replacement, does not cover the whole install, and its snapshot volumes are not part of the automated backup manifest above.
