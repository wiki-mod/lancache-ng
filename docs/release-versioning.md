...
- `utilities` (issue #1556; shared non-compiler CLI-tools image, consumed via
  ...
  own entry for the current `consumers:` list; curl moved from a
  self-compiled static binary to an apk-installed package, issue #1781, but
  is still one of the tools COPY'd cross-image, unchanged)
