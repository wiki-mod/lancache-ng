...
- Fluent-bit self-log rotation (#1236): `watchdog.sh`'s `maybe_rotate_fluent_bit_selflog()` bounds `/data/fluent-bit.log` (the `syslog` service's own operational log, see the logging matrix row below) on the `syslog-data` volume, which neither of the two mechanisms above touches. Deliberately has no `SYSLOG_ENABLED` gate (unlike the two engines above) and is not rate-limited by a daily stamp — checked every watchdog cycle instead, since a single-file `stat` is cheap and the growth this guards against is fast (confirmed empirically: roughly one line/second during a real syslog-ng outage). Once the file exceeds `FLUENT_BIT_SELFLOG_MAX_MB` (default 20), watchdog copies it aside (compressed with `zstd` when available), then truncates the live file in place ("copytruncate" — confirmed safe against the pinned fluent-bit image on a real runner: its `-l`/`--log_file` writes append-only, so an external truncate produces no data corruption or gaps) rather than renaming it away, since fluent-bit's `-l` has no signal- or rename-based reopen support at all (confirmed via its own `--help` output, AG-VAL-023). Rotated backups are themselves capped at `FLUENT_BIT_SELFLOG_MAX_ROTATIONS` (default 5, oldest deleted first), so total disk usage stays bounded regardless of how long an outage lasts.
