...
does not report individual option values. `scripts/tracked/simulations/dhcp-kea-lease-flow-simulation.sh`
...
`scripts/tracked/simulations/dhcp-proxy-pxe-simulation.sh` is the authoritative check for the
...
per its own design -- see `scripts/tracked/simulations/dhcp-proxy-pxe-simulation.sh`'s own
...
`scripts/tracked/simulations/dhcp-kea-ctrl-agent-mutation-simulation.sh` (issue #634) closes the
