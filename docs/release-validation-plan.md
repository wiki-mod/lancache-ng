...
| **CodeQL PR concurrency reclamation** (2026-08-08, AG-CI-017) | A newer commit on one PR cancels that PR's superseded CodeQL analysis per matrix language without cancelling the other language or any baseline-producing push run | Push two non-documentation commits to a scratch PR in quick succession. In the Actions UI or API, confirm the older run's `Analyze (actions)` and `Analyze (rust)` jobs are cancelled or complete before the newer jobs start; confirm both newer matrix jobs run; and confirm a concurrent `push`-event CodeQL run uses an independent group and completes. Repeat by rerunning the older PR run and confirm the rerun does not cancel the current commit's analysis. This scheduler behavior requires real GitHub Actions runs and cannot be established by local YAML lint alone | Fail if a superseded first-attempt PR analysis remains queued/running after its replacement starts, if the two current matrix languages cancel each other, or if a PR run cancels/blocks a push run or an isolated rerun |
