...
| Subsystem | What to check | How to check it for real | Pass/fail |
|---|---|---|---|
| **GHCR retention GC** | Protected `latest`, `nightly`, newest `minimum_stable_releases`, rollback anchors, within-budget ordinary roots, open/unknown PR versions, and shared artifact children survive; an aged beyond-budget ordinary root and its now-unprotected tagged closure are actually removed | First run `workflow_dispatch` with `dry_run=true` and record at least one concrete `would-delete` version ID. Confirm that same ID is absent from the audit's protected reasons. Then run the real GC with a deliberately small deletion cap and re-query that exact package version through the GitHub Packages API: it must return 404, while one sampled protected version ID still returns 200. During the run, move/add a protected tag to one planned candidate before its DELETE and confirm the immediate revalidation logs a keep and the version still returns 200. Run `bats tests/bats/gc_pr_staging_images.bats tests/bats/sha_retention_audit.bats` in the pinned build-tools container as the reusable negative-control suite. | Fail if a protected sampled ID is deleted, if an unchanged aged `would-delete` ID survives a real uncapped-by-that-candidate run without an explicit error/cap reason, if a changed-tag candidate is deleted, if worker concurrency loses/falsifies counters, or if any Bats case fails |
