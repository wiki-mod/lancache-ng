...
| **setup.sh `.env` migration idempotence (PR #1199 → CI 2.0 rewrite)** | `migrate_env_for_update()` is idempotent and convergent on real `.env` inputs (AG-OP-006/007), not just that a fixture parses | `bats .github/scripts/ci.bats` — the `migrate_env_for_update` behavior tests drive the real function twice and assert convergence and value-preservation. The former first-`@test` key-drift check (asserting `write_converged_env_fixture()` carried every historical key) was a fixture-coupled structure proxy and was dropped in the CI 2.0 rewrite; its real responsibility now lives in these behavior tests | Fail if any `migrate_env_for_update` behavior test fails, or if a real convergence/preservation property regresses |
