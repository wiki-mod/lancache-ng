...
| **CodeQL PR concurrency reclamation** (2026-08-08, AG-CI-017; workflow-level and PR-head-SHA corrections 2026-08-08) | A newer commit's push on one PR cancels that PR's entire superseded CodeQL run (both matrix legs together) without cancelling a baseline-producing push run, and the cancellation is ordered by run-creation time, not by which run's `detect-changes` job happens to resolve first. A current PR run compares `refs/pull/<N>/head` with `github.event.pull_request.head.sha`, not the synthetic merge commit in `github.sha`, and therefore reaches both CodeQL matrix legs | First push one non-documentation commit to a scratch PR and confirm its `detect-changes` job reports `superseded=false` and both CodeQL matrix legs are admitted. Then push a second non-documentation commit in quick succession. Confirm the older run (both `Analyze (actions)` and `Analyze (rust)`) is cancelled once the newer run starts, and that this holds even if the older run's `detect-changes` job is artificially delayed (e.g. by a concurrent burst of unrelated `lancache-light` work) so it would otherwise reach `analyze` after the newer run does. Confirm a concurrent `push`-event CodeQL run uses an independent group and completes. Repeat by rerunning the older PR run and confirm the rerun does not cancel the current commit's analysis. This scheduler and event-SHA behavior requires real GitHub Actions runs and cannot be established by local YAML lint alone | Fail if the current PR head is classified as superseded, either matrix leg is skipped for that reason, a superseded first-attempt PR run remains queued/running after its replacement starts, a delayed older run's late arrival cancels the newer run instead of the reverse, or a PR run cancels/blocks a push run or an isolated rerun |
