...
| **`ensure-pr-staging-images.sh` — ancestor fallback for a never-built PR base commit** (fixed, 2026-08-01, issue #1095 Part 1 follow-up, triggered by PR #1355) | An untouched service's back-fill genuinely succeeds once the PR base commit is confirmed to have never had a push-triggered `build-push.yml` run at all (a docs/governance-only commit skipped by that trigger's own `paths-ignore`, per issue #1095 Part 1) — WITHOUT weakening the #808/#626 fail-closed guarantee for a base commit whose build genuinely failed, is stuck, or is still running | Confirmed live: PR #1355 (`feat/watchdog`) had its "ensure PR staging images" job in FAILURE for 15+ hours because its base commit, `234f54a8` (PR #1363's merge commit, pure `docs(governance)`), structurally could never get a `sha-<short>` image built — `gh api repos/wiki-mod/lancache-ng/actions/workflows/build-push.yml/runs?head_sha=234f54a8acd13ea86a495e459af3306908f1a779&event=push` returns zero `workflow_runs`, confirmed against the real GitHub API while building the fix, versus exactly one run for a real built commit (`433c3fd2`, PR #1360) queried the same way. `scripts/ensure-pr-staging-images.sh` gained `base_commit_has_confirmed_push_run()` (positively confirms zero push runs before considering any fallback — any run, or an inconclusive check, preserves today's strict failure) and `find_built_ancestor()` (bounded ancestor walk, default 50 commits via `STAGING_ANCESTOR_SEARCH_DEPTH`, reusing `sif_wait_for_fresh_base_image()` against each run-bearing candidate, never skipping that freshness proof); the mutable `nightly`/`latest` channel tags are never touched — only one immutable per-commit tag is ever substituted for another. `bats tests/bats/ensure_pr_staging_images.bats` gained 6 new cases (ancestor 2 commits back found and used; bounded depth genuinely stops before reaching a further-back usable ancestor; no ancestor anywhere fails closed distinctly; a confirmed run on the base commit itself preserves the existing strict failure with no fallback attempted; an inconclusive run-check is treated the same as a confirmed run, not as "confirmed zero"). `bats tests/bats/ensure_pr_staging_images.bats` (all 18 cases, including the 6 new ones, must pass); `shellcheck --severity=warning scripts/ensure-pr-staging-images.sh tests/bats/ensure_pr_staging_images.bats` (must be clean); as a negative control, re-run the two `gh api ... runs?head_sha=...&event=push` queries above against the same two commits and confirm they still discriminate zero-vs-one — a check that stops discriminating silently reopens PR #1355's exact incident | Fail if the bats suite regresses (especially the "confirmed run -> no fallback" case, the load-bearing fail-closed guard), if `shellcheck` reports a new finding, or if either `gh api` control query stops discriminating built-vs-never-built commits |
...
- **`ensure-pr-staging-images.sh` ancestor fallback** (fixed, 2026-08-01, issue #1095
  Part 1 follow-up, triggered by PR #1355 — see the new Standing check row above) —
  `scripts/ensure-pr-staging-images.sh`'s untouched-service back-fill had no recovery
  path when a PR's base commit was itself a docs/governance-only commit that
  `build-push.yml`'s own push `paths-ignore` (#1095 Part 1) deliberately never
