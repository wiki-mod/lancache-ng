...
| `scripts/check-trivy-action-direct-usage.sh` | `aquasecurity/trivy-action` is only ever invoked through `.github/actions/trivy-scan-retry` (issue #1535, AG-CI-013), and every real call site of that wrapper sets both `dockerhub-username`/`dockerhub-password` (issue #1535 follow-up) |
