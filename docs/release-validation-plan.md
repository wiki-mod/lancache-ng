...
5. **What changed since the last validation, so re-validation can be scoped?** Run:
   ...
   DHCP-relevant":
   ```bash
   bash scripts/tracked/classify-image-impact.sh <last_validation.commit> <commit under test>
   ```
...
- `full-setup-deep-validate.yml`'s `pull_request` trigger **does** include
  ...
  <branch>` — or running the underlying `scripts/tracked/simulations/*-simulation.sh`
...
| **DHCP — dnsmasq relay** (new, PR #1117) | `dnsmasq-relay` mode genuinely **relays** (not just injects options) between two network segments | `bash scripts/tracked/simulations/dhcp-relay-flow-simulation.sh` (build-tools container / real Docker host) — this is the exact script #1117 used: two isolated bridges (client-net, server-net), a real `dhclient` DISCOVER on the client-net side, confirms the upstream DHCP server on the separate server-net received the request via the relay's `giaddr` and answered with a lease from the *client subnet's* pool. `tests/bats/setup_dhcp_mode.bats` for the mode-selection/config-render unit coverage | Fail unless the granted lease's subnet matches the client-side pool specifically (proves `giaddr` routing worked, not a coincidental same-subnet fallback) |
...
- **Real `setup.sh install` bring-up is the correct starting point for `deploy/quickstart`,
  ...
  `deploy/quickstart/scripts/tracked/docker-socket-proxy.sh` and
  ...
  **Recommended real bring-up sequence (documented here, not yet executed live as part of
  ...
  2. Feed that sequence into the same `expect`-based driving mechanism
     `scripts/tracked/simulations/setup-cli-simulation.sh` already uses in CI (`build_expect_prompt_block`,
     ...
     other work on a shared runner (the same reasoning `scripts/tracked/simulations/setup-cli-simulation.sh`
...
- **Image-freshness trap (confirmed live, 2026-07-24; local-build guidance updated for
  ...
  `scripts/tracked/simulations/*-simulation.sh` invocations too —
...
- **Wildcard/subdomain scope semantics (open gap, not yet closed by this pass —
  ...
  `scripts/tracked/simulations/proxy-deep-wildcard-tls-simulation.sh` and
  `scripts/tracked/simulations/proxy-standard-mode-sni-routing-simulation.sh` (both wired into
...
- **dnsmasq relay** (new): `bash scripts/tracked/simulations/dhcp-relay-flow-simulation.sh` — see Part A's
...
Run `scripts/tracked/simulations/nats-secondary-auth-callout-simulation.sh` end-to-end (this single script
...
Also run `scripts/tracked/simulations/ui-nats-dns-integration-simulation.sh` (added to this document
...
- **Admin UI self-restart (added 2026-08-19, issue #1486):** from `/setup`, submit
  ...
  of this pass** (tracked as a live-stack follow-up): `scripts/untracked/check-naming-consistency.sh`'s
...
- **Standing check (AG-VAL-029, added by issue #1343):** `scripts/tracked/simulations/setup-cli-simulation.sh`'s
...
validation coverage, ever" — `scripts/tracked/simulations/ntp-cap-sys-time-simulation.sh` already exists
...
- Reuse `scripts/tracked/simulations/ntp-cap-sys-time-simulation.sh` directly against the live stack
...
- DHCP dnsmasq-relay mode (PR #1117, closes #844) — first real relay mode alongside
  ProxyDHCP; `scripts/tracked/simulations/dhcp-relay-flow-simulation.sh` is its canonical proof.
...
- `setup.sh reset-to-last-known-good-config dns`/`pdns` (PR #1152, closes #836) — real
  CLI-driven PowerDNS zone rollback via `scripts/tracked/simulations/setup-reset-dns-config-simulation.sh`.
...
- Per-release CycloneDX SBOM + OpenVEX document (PR #1194, refs #1130) — drift guard
  (`scripts/untracked/check-vex-drift.sh`) already proven once against a real mismatch; the
...
- **Repo-rename case-sensitivity bug** (PR #1360, fixed, no dedicated tracking
  ...
  `scripts/tracked/select-build-tools-image.sh`'s fork-vs-same-repo trust check), each
...
- **Untouched-service staging back-fill ancestor fallback** (fixed, 2026-08-01, issue
  ...
  above) — `scripts/tracked/ensure-pr-staging-images.sh`'s untouched-service back-fill had no
...
- **pipefail/SIGPIPE early-exit-consumer bug in `tools/build-tools/Dockerfile`**
  ...
  (`scripts/tracked/check-pipefail-early-exit-grep.sh`, wired into the
...
- **F-17 SHA retention audit** (issue #1095, PR #1501, new — protect-only, no
  ...
  `scripts/tracked/gc-sha-retention-audit.sh`, `scripts/lib/sha-retention-
...
- **`current-dev-auto-close.yml` closing-keyword scanner reading the whole PR
  ...
  this project's build-tools image, and `scripts/untracked/validate-pr-template.sh`
...
- **Corrected 2026-08-05 (issue #1391 audit) — this entry was itself stale.** A prior
  ...
  enforced project-wide by `scripts/untracked/check-idempotence-test-coverage.sh`) —
...
- **Proxy `slice` + a cached 301/302 response has never been exercised end-to-end**
  ...
  - **Tracking**: bug-hunt #849 finding #14 (2026-08-06). Follow-up: a
    dedicated `scripts/tracked/simulations/proxy-slice-redirect-cache-simulation.sh`
  ...
  - **Validation**: No automated coverage exists for this exact path today.
    `scripts/tracked/simulations/ssl-mitm-cache-simulation.sh`'s own comment explains why *its*
...
- **Five gaps confirmed by this audit pass (2026-08-05, issue #1391),
  ...
  recommended procedure (reusing `scripts/tracked/simulations/setup-cli-simulation.sh`'s own `expect`
...
- **AG-VAL-034's GNU/BusyBox construct audit for Alpine-migrated services has no
  ...
  same shape as `scripts/tracked/check-pipefail-early-exit-grep.sh`'s existing pattern/fix-package
...
- **Recorded exception (2026-08-07): no mechanical guard against a
  ...
  - **Validation**: found and fixed only by a human/agent reading the
    ...
    exception covers -- `scripts/untracked/check-dependabot-docker-base-consistency.sh`
...
- **Recorded exception (2026-08-22, issue #1095 G15, PR #1640, commit
  ...
  - **Scope**: a maintainer-directed repo-wide sweep (AG-WF-011/AG-CI-015 --
    ...
    not a hypothetical); and `scripts/tracked/docker-socket-proxy.sh`'s
  ...
  - **Reason**: (4) and (5) are genuinely drift-prone by construction --
    ...
    other `scripts/untracked/*.sh` owns either shape. Per AG-CODE-013, creating
  ...
  - **Validation**: manual only. A standalone, non-Dockerfile reproduction of
    ...
    additionally checked against `scripts/untracked/check-naming-
...
|---|---|
| `scripts/tracked/simulations/dhcp-relay-flow-simulation.sh` | Real two-segment DHCP relay (PR #1117) |
| `scripts/tracked/simulations/nats-secondary-auth-callout-simulation.sh` | NATS secondary lifecycle, active-disconnect (`connz`/`KICK`), xkey encryption (packet capture + negative control) |
| `scripts/tracked/simulations/setup-reset-dns-config-simulation.sh` | Real CLI-driven PowerDNS zone rollback (PR #1152) |
| `scripts/tracked/simulations/setup-reset-kea-config-simulation.sh` | Real CLI-driven Kea config rollback |
| `scripts/tracked/generate-vex.sh` / `scripts/untracked/check-vex-drift.sh` | OpenVEX document reproducibility and drift detection (PR #1194) |
| `tests/bats/setup_update_idempotence.bats` (first `@test`) | `.env` key-drift guard (PR #1199) |
| `scripts/untracked/check-idempotence-test-coverage.sh` | Every stateful config-writer has repeat-run/idempotence test coverage |
| `scripts/untracked/check-bats-path-filter-coverage.sh` | Every real bats dependency is covered by `build-tools.yml`'s path filters |
| `scripts/untracked/check-workflow-service-lists.sh` | Hardcoded service arrays stay in sync across workflow files |
| `scripts/untracked/check-naming-consistency.sh` | Container-name/allowlist/env-var naming contract (`docs/naming-conventions.md`) |
| `scripts/untracked/check-file-headers.sh` | File-header contract (`AG-HDR-*`) |
| `scripts/tracked/check-trivy-action-direct-usage.sh` | `aquasecurity/trivy-action` is only ever invoked through `.github/actions/trivy-scan-retry` (issue #1535, AG-CI-013, quoted or unquoted `uses:` scalars alike), and every real call site (`.github/workflows` and `.github/actions`) sets both `dockerhub-username`/`dockerhub-password` to a real, non-empty `secrets.*`/`inputs.*` reference, not merely present (issue #1535 follow-up) |
| `scripts/tracked/classify-image-impact.sh` | The single source of truth for "which subsystem does this diff touch" — reused by this document's own staleness reasoning, `detect-changes`, and the `promote` job's version-bump logic |
| `scripts/tracked/validate-stack-images.sh` | Release-notes/workflow status-line consistency |
| `scripts/tracked/select-build-tools-image.sh` | Resolves the pinned build-tools image/digest for every container-based check above |
| `scripts/tracked/simulations/full-setup-client-simulation.sh` | Full-setup harness client-side probe |
