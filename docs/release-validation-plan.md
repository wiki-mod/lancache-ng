...
| **CodeQL PR concurrency reclamation** (2026-08-08/09, AG-CI-017) | First-attempt PR analyses are serialized per PR and matrix language with `cancel-in-progress: false`, so one busy PR cannot admit an unbounded number of jobs to the heavy runner pool and an older waiter can never cancel a newer run. Before entering that queue, `detect-changes` resolves the authoritative `refs/pull/<N>/head` with a bounded four-attempt retry and skips a superseded run. Push, schedule, dispatch, and rerun analyses use isolated concurrency keys. | Push a commit that changes `services/watchdog/src/main.rs` (or another path classified as Rust-relevant) to a scratch PR. Confirm `detect-changes` reports both `rust_relevant=true` and `superseded=false`; confirm both matrix jobs are created, `Analyze (actions)` performs Actions analysis, and `Analyze (rust)` performs Rust initialization, builds, and analysis. While those jobs run, push another Rust-relevant commit and confirm the newer run's corresponding matrix legs remain queued behind the older legs rather than running concurrently or cancelling either run. Push a third Rust-relevant commit while the second is queued; confirm its `detect-changes` reports `superseded=false`, while the second run is detected as stale before heavy analysis when its head lookup occurs after the third push. Also force the remote-head command to fail transiently and confirm it retries four times before the workflow's deliberate fail-open behavior. Repeat by rerunning an older attempt and confirm its run-id-scoped key neither blocks nor cancels the current first-attempt analysis. Scheduler behavior requires real GitHub Actions runs; locally run the resolver bats tests and actionlint. | Fail if more than one first-attempt analysis for the same PR/language is admitted concurrently, any concurrency event cancels a run, a transient lookup gets only one attempt, a current PR head is classified as superseded, the Rust-relevant validation does not execute actual Rust analysis, or a push/schedule/dispatch/rerun shares the PR serialization key. |
