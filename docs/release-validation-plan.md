...
| **NATS — Admin UI DNS-sync propagation** (added 2026-08-05, issue #1391 audit — previously wired into CI but never referenced in this document, and a genuinely different NATS function than secondary-lifecycle) | A DNS entry created via the Admin UI is really propagated end-to-end: UI → NATS → `nats-subscriber` → PowerDNS, and is genuinely resolvable, not just accepted by the UI | `full-setup-sims.yml`'s `ui-nats-dns-integration-simulation` job (`scripts/untracked/simulations/ui-nats-dns-integration-simulation.sh`, issue #400) — creates a real entry via the Admin UI, confirms it lands in PowerDNS via a real `dig`, then removes it and confirms removal propagates too (on the AXFR secondary the removal is proven by authoritative zone convergence first, then a recursor drop within the cached TTL, so TTL expiry cannot masquerade as a working flush) — already CI-wired, reuse rather than re-derive | Fail if the entry never resolves via `dig` after creation, or still resolves after removal |
...
| **Alpine build-tools native Rust contract** (Issue #1860) | The sole Alpine build-tools image provides the native Rust toolchain and the service builders compile for the installed Alpine host target rather than an unavailable rustup target | Build `tools/build-tools/Dockerfile` for each published architecture; run its required-tool self-check and `.github/actions/build-tools-candidate-smoke`; build at least one Rust service with the exact candidate image and verify `rustc -vV` reports the architecture's `*-alpine-linux-musl` target | Fail if a build-tools stage uses Debian or `rust:*`, if a required tool is absent, if the candidate smoke suite fails, or if a service builder calls `rustup target list` or cannot compile for its native target |
...
**New standing check (AG-VAL-029, added 2026-08-27, Issue #1613/#1095): release-sbom utilities-component over-reporting** -- `release-sbom`'s "Merge utilities components into this service's SBOM" step used to merge services/utilities' ENTIRE CycloneDX SBOM into every one of the 7 consumers' (`proxy`/`dhcp`/`dhcp-proxy`/`dns`/`ntp`/`ui`/`watchdog`) own published SBOM unconditionally, even though each consumer's Dockerfile `COPY --from=utilities-tools` only a small subset of utilities' installed apk packages (e.g. `ui` copies only `lsof`/`rg`/`libgcc_s.so.1`/`libpcre2-8.so.0*`, never `nano`/`coreutils`/`jq`/`zstd`/`findutils`/`gettext-envsubst`). Fixed by extracting the merge into `scripts/untracked/merge-utilities-sbom-components.sh`, which filters utilities' components to a per-service apk-package allowlist (derived from each `services/*/Dockerfile`'s COPY lines and verified live via `apk info -L <pkg>` against a real, freshly-pulled `ghcr.io/wiki-mod/lancache-ng/utilities` image on a real self-hosted runner -- not reasoned from memory). `curl` is COPY'd by several consumers too but is built from source in `services/utilities/Dockerfile`'s `curl-builder` stage, never `apk add`ed, so it has no apk-db SBOM component at all and needed no allowlist entry -- confirmed live (`apk info --who-owns /usr/local/bin/curl` -> "Could not find owner package") against the same real image; the merge step's prior comment claiming curl needed "recovering" via this merge was incorrect and has been corrected. The `release-sbom` job itself is gated `startsWith(github.ref, 'refs/tags/v')` and cannot be exercised by PR CI, so the durable regression coverage lives in the bats suite below, run in the pinned build-tools container against fixture CycloneDX documents shaped like real Trivy output (confirmed live: `jq -c '.components[] | {name, type, group, purl}'` against a real `trivy image --format cyclonedx` scan of the utilities image shows `name` is the bare apk package name, e.g. `"findutils"`/`"libintl"`/`"pcre2"`, matching the fixtures) -- plus a live end-to-end run of the script against that same real Trivy-scanned utilities SBOM for `ui`, which produced exactly `libgcc lsof pcre2 ripgrep` and nothing else:
