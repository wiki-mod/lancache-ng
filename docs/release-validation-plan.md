...
| **Review-chronology code comments (AG-CODE-003 sub-pattern)** (2026-08-02; enforcement model updated 2026-08-26, issue #1095, PR #1686) | No git-tracked source/config comment narrates the review chronology of the change it sits in ("caught in review", "before this fix", "flagged in review on PR #123", and similar phrasings) — these read as internal notes to the reviewer/author at PR-open time, not durable technical rationale, and go stale/confusing the moment the PR merges | Two-tier enforcement, both via `scripts/untracked/check-review-chronology-comments.sh`, wired into `build-push.yml`'s `file-headers`/`file-headers-hosted` jobs: (1) a repo-wide baseline (`.github/actions/file-headers-check/action.yml`, `CHRONOLOGY_WARN_ONLY=1`) reports every finding anywhere in the tree as `::warning::` but never fails the job — this catches drift without letting a pre-existing violation elsewhere block an unrelated PR; (2) a diff-scoped step in the same jobs (`CHRONOLOGY_DIFF_BASE_SHA`/`CHRONOLOGY_DIFF_BASE_REF`) restricts the scan to exactly the files the PR's own diff touched and fails the job on a finding there. `bats tests/bats/check_review_chronology_comments.bats` (38 cases, covering both the original real-shaped positive/negative fixtures and the explicit-file/warn-only/diff-scoped modes added by PR #1686) | Fail if the diff-scoped step reports a finding against a file this PR's own diff touched, or if any bats case regresses (especially the original negative-fixture cases — a widened regex that starts flagging generic "review"/"PR" mentions is a false-positive regression, not an improvement). The repo-wide baseline reporting a pre-existing violation elsewhere as `::warning::` is expected, by design, and must not be read as a passing job masking a real finding |
