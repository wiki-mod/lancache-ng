...
| **CodeQL PR concurrency reclamation** (2026-08-08, AG-CI-017; workflow-level, PR-head-SHA, and no-concurrency-block corrections 2026-08-08/09) | Every CodeQL run (PR, push, schedule, dispatch) starts immediately and independently — there is no `concurrency:` block, so no run can ever cancel or queue behind another — and each run's own `detect-changes` job makes a live, authoritative "am I superseded" call before the expensive `analyze` job starts, comparing `refs/pull/<N>/head` with `github.event.pull_request.head.sha` (not the synthetic merge commit in `github.sha`), so a current PR head is never misclassified as superseded. Push runs are excluded from this self-skip entirely, so a paths-ignored Markdown-only follow-up push can never wrongly mark an actual code-change push as superseded and leave it unscanned | Push one non-documentation commit to a scratch PR and confirm its `detect-changes` job reports `superseded=false` and both CodeQL matrix legs run. Then push a second non-documentation commit in quick succession and confirm both runs' workflows start immediately (neither shows "Queued" or "Cancelled" in the Actions run list). Confirm the older run's own `detect-changes` reports `superseded=true` and both `Analyze (actions)`/`Analyze (rust)` matrix legs are skipped as a job-level skip, while the newer run's `detect-changes` reports `superseded=false` and both legs execute — including when the older run's `detect-changes` is artificially delayed past the newer run's completion. Then push one non-documentation commit followed immediately by a Markdown-only commit to the same scratch PR and confirm the first push's own PR run still completes both matrix legs (not skipped). Confirm an independent `push`-event CodeQL run to a real branch is never marked superseded by a later paths-ignored push to that branch. Repeat by rerunning an older PR attempt and confirm it neither affects nor is wrongly treated as the current commit's analysis. This scheduler and event-SHA behavior requires real GitHub Actions runs and cannot be established by local YAML lint alone. Note: if two PR pushes land close enough together that the older run's `detect-changes` completes before the newer push registers on the remote, both runs' `analyze` jobs proceed and briefly run the heavy Rust leg concurrently on the same self-hosted pool — this is an accepted, bounded-waste tradeoff of dropping the concurrency block (see that block's removal comment), not a regression to flag | Fail if any run is queued or cancelled by a concurrency group (none should exist), if the current PR head is ever classified as superseded, if either matrix leg is skipped on the actual latest commit for that reason, if a push run's analysis is skipped due to a later paths-ignored push, or if a rerun of an older PR attempt affects the current commit's analysis |
