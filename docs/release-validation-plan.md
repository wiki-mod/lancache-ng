...
| **CodeQL PR concurrency reclamation** (2026-08-08, AG-CI-017; workflow-level correction 2026-08-08) | A newer commit's push on one PR cancels that PR's entire superseded CodeQL run (both matrix legs together) without cancelling a baseline-producing push run, and the cancellation is ordered by run-creation time, not by which run's `detect-changes` job happens to resolve first | Push two non-documentation commits to a scratch PR in quick succession. Confirm the older run (both `Analyze (actions)` and `Analyze (rust)`) is cancelled once the newer run starts, and that this holds even if the older run's `detect-changes` job is artificially delayed (e.g. by a concurrent burst of unrelated `lancache-light` work) so it would otherwise reach `analyze` after the newer run does. Confirm a concurrent `push`-event CodeQL run uses an independent group and completes. Repeat by rerunning the older PR run and confirm the rerun does not cancel the current commit's analysis. This scheduler behavior requires real GitHub Actions runs and cannot be established by local YAML lint alone | Fail if a superseded first-attempt PR run remains queued/running after its replacement starts, if a delayed older run's late arrival cancels the newer run instead of the reverse, or if a PR run cancels/blocks a push run or an isolated rerun |
