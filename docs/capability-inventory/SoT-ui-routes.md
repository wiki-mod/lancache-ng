...
> against current `origin/v0.2.0` code. (1) `scripts/untracked/simulations/syslog-forwarding-simulation.sh`
...
> count corrected from 11 to the real 12 (`ls scripts/untracked/simulations/*-simulation.sh`
> post-F-16 restructure; `ls scripts/*-simulation.sh` at the time this note
> was written), since the original count missed this script. (That count
> itself is now further stale for an unrelated reason: this repo has kept
> adding simulation scripts since 2026-07-18, and the real current count is
> higher than 12 -- re-auditing route coverage against the full current set
> is outside this specific path-rename fix's scope, flagged here rather
> than silently left implying 12 is still accurate.) (2) `register_secondary` in
...
Scope: exhaustive pass over every file in `services/ui/src/routes/` (10 files, all read in full against `origin/v0.2.0`), cross-referenced against `services/ui/src/main.rs`'s route table (the actual HTTP method+path wiring, since the route modules themselves don't declare their own paths), `services/ui/src/templates/` (8 templates), the 12 `scripts/*-simulation.sh` E2E scripts at audit time (the F-16 restructure later moved these to `scripts/untracked/simulations/*-simulation.sh`, and more have been added since -- see this file's own currency-check note above), and the current open-issue list. Governance read in full for this pass: `.github/AGENTS.md` (all 452 lines, including the Rule Enforcement Matrix), `CLAUDE.md` (both the stale copy on `fix/sccache-fallback-exit-code` and the current `origin/v0.2.0` version -- they differ meaningfully, e.g. `origin/v0.2.0` describes a unified single `services/proxy` container with port-based mode split, not two separate proxy services), and `CONTRIBUTING.md` (`origin/v0.2.0`, 299 lines).
...
- `add_subnet_option()` / `remove_subnet_option()`: per-subnet arbitrary DHCP option management (**this is exactly the "custom DHCP-option management" issue #556 asked for**) -- validated code (1-254, excluding the 5 UI-managed codes) and data (non-empty, ≤1024 bytes, single-line). **Update (2026-07-23, issue #1085):** the "code" field now also accepts the three PXE top-level subnet keys `next-server`/`server-hostname`/`boot-file-name` (parsed by `parse_custom_option_key`); these route to the subnet's top-level JSON keys instead of `option-data`, with `next-server` validated as a bare IPv4 and the two string fields bounded to their BOOTP sizes (64 / 128 bytes). The Admin UI table/dropdown that surfaces these to operators is deferred to the DHCP mockup rework (umbrella #1078); this was a backend-routing change only.
