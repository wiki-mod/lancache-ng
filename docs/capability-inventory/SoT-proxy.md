...
| Stream-target map generation → `stream.d/00-stream-targets.conf` | `map $ssl_preread_server_name $stream_backend` for SNI passthrough — lazy mode forwards blind to whatever SNI the client sent, strict mode only forwards to domains in the allowlist (else routes to a closed `127.0.0.1:9`). **Registrable-root target-selection gap (confirmed 2026-07-18, tracked as issue #1297):** for a matched registrable root, `entrypoint.sh` still forwards to `${domain}:443` — the *derived root* domain — not to the literal `$ssl_preread_server_name` the client requested. E.g. a listed `drivers.amd.com` (PSL root `amd.com`) forwards to `amd.com:443`, not `drivers.amd.com:443`, which can break strict-mode standard-mode traffic for any listed subdomain whose real origin doesn't serve that traffic from the root domain's IP. **A deeper leading-dot wildcard base (issue #1272) does NOT have this gap** — the entrypoint forwards those to `$ssl_preread_server_name:443` correctly; only the older registrable-root case (issue #1297) remains open. | Real-handshake-verified for the #1272 wildcard-base case via `scripts/proxy-deep-wildcard-tls-simulation.sh` (grep-checks the generated stream-target map directly); the pre-existing #1297 registrable-root case has no isolated test either way |
