...
| Test | What it actually proves | Count |
|---|---|---|
| `tests/bats/dhcp_kea_config_generation.bats` | First-boot NTP resolution/validation functions, DDNS reverse-zone list stays in sync with `PRIVATE_REVERSE_ZONES`, rendered JSON parses | 22 |
| `tests/bats/dhcp_lease_flow_parsing.bats` | Lease-flow output parsing helpers | 7 |
| `tests/bats/setup_dhcp_mode.bats` | `setup.sh`'s DHCP-mode selection/env wiring | 9 |
| `tests/bats/shared_secret_bootstrap.bats` | `secret_is_placeholder`/`resolve_shared_secret` logic in isolation | 7 |
| `tests/bats/shared_secret_bootstrap_sync.bats` | Byte-identity of the embedded copy in this file vs. the canonical library and the two sibling copies | 4 |
| `dhcp.rs` `#[cfg(test)]`/`#[tokio::test]` (in-file) | `kea_config_modify`'s success/rollback/retry/ambiguous-failure matrix, global-reservation-identifiers guard, `lease4-del` result-code handling, CSRF/mode gating, validation helpers | 70 |
| `kea_snapshots.rs` `#[test]` (in-file) | Snapshot creation, retention, rollback ID membership | 8 |
