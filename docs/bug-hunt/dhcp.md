...
- **N3. `KEA_CTRL_TOKEN` healthcheck placeholder-detection fallback is
  inconsistent across the three compose files and does not fully mirror
  `secret_is_placeholder()`'s case-insensitive/dash-equivalent matching
  ...
  Concretely: if an operator configures `KEA_CTRL_TOKEN` to any
  placeholder-shaped value that `entrypoint.sh` correctly recognizes and
  replaces with the real shared-secret-generated token (a case variant, a
  ...
  This exact gap was already flagged (but never filed) in a comment on
  ...
  **Evidence:** `deploy/dev/docker-compose.yml:312`,
  ...
  `deploy/prod/docker-compose.yml:347`; `services/dhcp/entrypoint.sh`
  `secret_is_placeholder()` (lines 89-95); `deploy/dev/.env:13`
  (`KEA_CTRL_TOKEN=lancache-dev-kea-control-token-change-me`, real, not a
  placeholder by this function's own documented, intentional exclusion of
  the infix `change-me` shape); `deploy/prod/.env:51` and
  `deploy/quickstart/.env:71` (`KEA_CTRL_TOKEN=CHANGE_ME_KEA_CTRL_TOKEN`,
  the one literal value all three healthcheck variants happen to already
  cover, which is why this gap has not yet surfaced as a live incident).
