...
# scripts/tracked/check-netdata-curl-pin.sh's fetch_bundled_packages_version() read
...
for file in "${yaml_scan_files[@]}"; do
  ...
  while IFS=: read -r lineno finding; do
    [ -n "$lineno" ] || continue
    fail "$file:$lineno: $finding. See scripts/tracked/check-pipefail-early-exit-grep.sh's own header (THIRD, INDEPENDENT CHECK) for the confirmed real incident this guards against."
