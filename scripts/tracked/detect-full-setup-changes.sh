...
#
# scripts/untracked/ (issue #1095, executed post-v0.3.0-release): the 24
...
# since actually moved into scripts/untracked/ (git mv, not a fresh file) and
# been dropped from the array below, which is now empty by design: ANY path
# under the scripts/untracked/ prefix is recognized as CI-tooling-only,
...
# directory/prefix" below, because scripts/untracked/ is itself defined to
...
# scripts/untracked/ at creation, not by adding a new array entry here.
#
...
# scripts/untracked/ has a documented, tested fallback path) may only ever be
...
# scripts/ path that is neither under scripts/untracked/ nor listed here (a
# brand-new script, an unclassified one, anything under scripts/tracked/,
...
touches_scripts_beyond_ci_tooling_allowlist() {
    local path allowed known classification found_beyond=1
    while IFS= read -r path; do
        [[ "$path" == "scripts/"* ]] || continue
        allowed=false
        if [[ "$path" == "scripts/untracked/"* ]]; then
            allowed=true
        else
        ...
        # What: emits which classification a changed script path hit.
        # Why: makes should_run's verdict traceable per path, not just the
        # aggregate result -- see AGENTS.md Rule-Ref: AG-CI-* observability.
        # From: Issue #1095
        classification="Untracked (CI-tooling-only, exempt)"
        [[ "$allowed" == "false" ]] && classification="Tracked (should_run-relevant)"
        echo "::notice::${path}: ${classification}" >&2
        [[ "$allowed" == "false" ]] && found_beyond=0
    done < "$changed_files"
    return "$found_beyond"
}
