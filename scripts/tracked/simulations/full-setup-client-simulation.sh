...
# scripts/tracked/simulations/ssl-mitm-cache-simulation.sh's own `docker compose up` call), so
...
# Unlike scripts/tracked/simulations/ssl-mitm-cache-simulation.sh (which sets up the Compose
...
docker run --rm \
    ...
    bash -ceu '
        ...
        # scripts/tracked/simulations/ssl-mitm-cache-simulation.sh names it explicitly), so this
        ...
        # listener) lives in scripts/tracked/simulations/ssl-mitm-cache-simulation.sh, which does
        # start the shim.
