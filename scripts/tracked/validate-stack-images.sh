...
# service:` build-matrix entry (so scripts/untracked/check-workflow-service-lists.sh's
...
require_grep 'imagetools inspect "\$image" --format' \
  scripts/tracked/require-image-platforms.sh \
...
if awk '!/^[[:space:]]*#/ && /(^|[^[:alnum:]_])jq([[:space:]]|$)/ { found=1 } END { exit found ? 0 : 1 }' "$repo_root/scripts/tracked/require-image-platforms.sh"; then
...
require_grep 'bash scripts/tracked/require-image-platforms\.sh "ghcr\.io/\$\{REPOSITORY\}/\$\{service\}@\$\{expected_digest\}" "\$REQUIRED_PLATFORMS"' \
  .github/workflows/build-push.yml \
...
require_grep 'stable releases require external images in supported profiles to be pinned by digest, mirrored, or explicitly removed from the stable profile' \
  scripts/untracked/check-stable-external-images.sh \
  'stable release promotion must fail closed while release-relevant external images are floating'
require_grep 'bash scripts/untracked/check-stable-external-images.sh' \
  .github/workflows/build-push.yml \
...
require_grep 'bash scripts/tracked/require-image-platforms\.sh "\$image" "\$REQUIRED_PLATFORMS"' \
  .github/workflows/build-push.yml \
