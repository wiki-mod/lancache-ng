...
# The proxy Dockerfile consumes the DNS domain list as a named build context.
# A domain-list-only change therefore has to rebuild both DNS and proxy or the
# proxy image would retain an older baked-in copy of that list.
