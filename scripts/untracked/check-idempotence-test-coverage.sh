...
#
# Usage:
#   scripts/untracked/check-idempotence-test-coverage.sh [repo_root]
set -euo pipefail
...
for pair in "${WRITER_TEST_EVIDENCE[@]}"; do
    ...
    if [ ! -f "$writer_path" ]; then
        fail "check-idempotence-test-coverage: config-writer '$writer_path' no longer exists; update WRITER_TEST_EVIDENCE in scripts/untracked/check-idempotence-test-coverage.sh (or remove the entry if the writer itself was removed)."
        continue
    fi
    ...
    case "$evidence_path" in
        ...
        *.rs)
            ...
            ;;
        *)
            fail "check-idempotence-test-coverage: unsupported evidence file type for '$evidence_path' (expected .bats or .rs); update scripts/untracked/check-idempotence-test-coverage.sh."
            ;;
    esac
done

if [ "$failures" -gt 0 ]; then
    printf '::error::check-idempotence-test-coverage: %d config-writer(s) missing repeat-run/idempotence test coverage (see scripts/untracked/check-idempotence-test-coverage.sh).\n' "$failures" >&2
    exit 1
fi
