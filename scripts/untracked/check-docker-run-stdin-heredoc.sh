...
#
# SCOPE: workflow YAML under .github/workflows/**/*.yml and composite
# actions under .github/actions/**/*.yml -- the two places this repo defines
...
while IFS= read -r -d '' file; do
  ...
  while IFS=: read -r lineno _rest; do
    ...
    # actual invocation this heredoc belongs to.
    invocation="$(printf '%s\n' "$window" | sed -n '/docker run/,$p')"
...
done < <(find .github/workflows .github/actions -type f -name '*.yml' -print0 2>/dev/null)
