...
#   - scripts/tracked/gc-pr-staging-images.sh: a `services=(...)` copy that must
...
#   - scripts/tracked/ensure-pr-staging-images.sh: a `full_setup_services=(...)`
...
if [[ -n "${1:-}" ]]; then
    ...
    extra_files=(
        "scripts/tracked/gc-pr-staging-images.sh"
...
# scripts/tracked/gc-pr-staging-images.sh), which must always equal the full set. Keyed by
...
# scripts/tracked/gc-pr-staging-images.sh, backfill-stack-latest.yml) so a rename/refactor
...
declare -A FULL_SETUP_EXACT_EXCLUSIONS=(
    ...
    # scripts/tracked/simulations/syslog-forwarding-simulation.sh's Triggers 7/8 (added by #864,
    # after this exclusion set was first written) pull both images directly.
    # ntp (the last remaining member) moved out too once
    # scripts/tracked/simulations/syslog-forwarding-simulation.sh gained a real ntp
