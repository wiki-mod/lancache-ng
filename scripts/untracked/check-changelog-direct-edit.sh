...
# CONTRIBUTING.md itself documents -- scripts/tracked/collect-changelog-entries.sh's
...
main() {
  ...
  if pr_has_release_label; then
    echo "::notice::This PR edits CHANGELOG.md and carries the 'release' label, so this looks like an expected manual release-notes edit (e.g. the scripts/tracked/collect-changelog-entries.sh fallback path). No action needed."
  else
