...
#
# Enforces AG-REL-001 (project language: Rust and shell for code we write,
...
# frontend charting library) is not "code we write" -- AG-REL-001 governs
...
# in (AG-REL-001). Extend this list if a new banned-language incident occurs
...
if [ "${#offenders[@]}" -gt 0 ]; then
    echo "Files in a language this project has decided not to write in were found (AG-REL-001 violation):" >&2
    printf '  %s\n' "${offenders[@]}" >&2
    echo "" >&2
    echo "Project language is Rust (and shell for scripts). Introducing another language requires" >&2
    echo "explicit maintainer approval every time, per AG-REL-001 in AGENTS.md -- it is not a blanket" >&2
...
echo "No files in a banned language found -- AG-REL-001 holds."
