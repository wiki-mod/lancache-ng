...
# checkable: that the Docker socket proxy allowlist (scripts/tracked/docker-socket-proxy.sh),
...
# see scripts/tracked/docker-socket-proxy.sh's own comment on that addition. This
...
SOCKET_PROXY_SCRIPT=scripts/tracked/docker-socket-proxy.sh
