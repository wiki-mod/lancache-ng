...
#
# Usage:
#   scripts/untracked/check-validation-subnet-wrapper-coverage.sh [repo_root]
set -euo pipefail
...
for file in "${WORKFLOW_FILES[@]}"; do
    if [[ ! -f "$file" ]]; then
        fail "check-validation-subnet-wrapper-coverage: '$file' no longer exists; update WORKFLOW_FILES in scripts/untracked/check-validation-subnet-wrapper-coverage.sh."
        continue
    fi
    check_workflow_file "$file"
done
...
if [[ "$failures" -gt 0 ]]; then
    printf '::error::check-validation-subnet-wrapper-coverage: %d violation(s) found (see scripts/untracked/check-validation-subnet-wrapper-coverage.sh).\n' "$failures" >&2
    exit 1
fi
