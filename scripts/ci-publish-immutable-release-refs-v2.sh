...
if [[ $# -ne 3 ]]; then
    echo "usage: ci-publish-immutable-release-refs-v2.sh RELEASE_LOCK RELEASE_TAG STACK_DIGEST" >&2
    exit 2
fi
...
# Phase 1: establish the complete known registry state before the first write.
# This makes a conflicting existing immutable tag fail before an earlier
# package ref can be created by this run.
for row in "${desired_rows[@]}"; do
    ...
    if actual="$(ci_ai_ref_digest_optional "$target")"; then
        ...
        if (( status == CI_AI_REF_ABSENT_STATUS )); then
            ...
            printf 'ci-artifact-identity: could not establish immutable release ref state for %s\n' "$target" >&2
            exit "$status"
        fi
    fi
done
...
# are valid pieces of the same release and can be reused by an idempotent retry;
