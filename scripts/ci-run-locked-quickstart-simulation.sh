...
# The caller owns Quickstart's host-local mutex. In the V2 workflow it is
# acquired before entering this immutable build-tools container and therefore
# covers this script's complete lifetime on the shared Docker host. Taking the
# same flock again from a nested container would self-deadlock instead of
